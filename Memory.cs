...
namespace ClashConfig {
 public static class LocalMemory {
  ...
  public static void Test(string report){
   ...
   try{if(Load()!=null)throw new Exception("Initial state");var input=new Input{Kind="SOCKS5",Address="localhost:1080",Username="test-user",Password="test-only-secret",Paths=new[]{@"C:\test\app.exe"},AutoStart=true,RuleMode=2,NodeName="Test-node"};Save(input);var restored=Load();if(restored.Password!=input.Password||restored.Paths[0]!=input.Paths[0]||!restored.AutoStart||restored.RuleMode!=2||restored.NodeName!="Test-node")throw new Exception("Round trip");if(Encoding.UTF8.GetString(File.ReadAllBytes(FilePath)).Contains(input.Password))throw new Exception("Plaintext");input.Password="updated";Save(input);if(Load().Password!="updated")throw new Exception("Replace");File.WriteAllBytes(FilePath,new byte[]{1,2,3});bool rejected=false;try{Load();}catch(CryptographicException){rejected=true;}if(!rejected)throw new Exception("Corrupt payload");Clear();if(Load()!=null)throw new Exception("Clear");File.WriteAllText(report,"PASS: encrypted persistence, credentials and app-path round trip, atomic replacement, corrupt-file rejection and forgetting.");}finally{if(Directory.Exists(Core.Data))Directory.Delete(Core.Data,true);Core.Data=old;}
  }
 }
}
