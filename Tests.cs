...
namespace ClashConfig {
 public static class Tests {
  ...
  public static void Run(string report){
   Guard.CheckLayout();ModeRulesTest();string path=@"C:\Program Files\测试软件\app.exe";
   ...
   File.WriteAllText(report,"PASS: mode2 selected-only name/directory matching, sniffer, direct fallback, fixed node selection; x64 WFP layouts; no default selection; URL parsing; IPv6 input; credential YAML escaping; exact process routes; reject after proxy; unknown-process deny; non-target direct; direct-node and path injection rejection; subscription inbound sanitization; backup restoration of existing and originally absent files.");
  }
  static void ModeRulesTest(){
   string edge=@"C:\Apps\Edge\msedge.exe",douyin=@"C:\Program Files (x86)\ByteDance\douyin\1.2\douyin.exe";
   var edgeRules=Mode2.Rules(new[]{edge});Assert(edgeRules.Last()=="MATCH,DIRECT","Mode2 default direct");Assert(!edgeRules.Any(r=>r.Contains("douyin")),"Unselected app included");Assert(edgeRules.Contains("PROCESS-NAME,msedge.exe,"+Core.Group),"Mode2 process name missing");
   var dy=Mode2.Rules(new[]{douyin});Assert(dy.Contains(@"PROCESS-PATH-WILDCARD,C:\Program Files (x86)\ByteDance\douyin\*,"+Core.Group),"Directory matching missing");Assert(dy.Contains("PROCESS-NAME-WILDCARD,*douyin*,REJECT"),"Mode2 UDP reject missing");Assert(Mode2.Matches(@"C:\Program Files (x86)\ByteDance\douyin\2.0\helper.exe",new[]{douyin}),"Mode2 helper verification missing");
   string raw="proxies:\n- {name: Generic-2, type: socks5, server: 127.0.0.1, port: 1081}\n- {name: Generic-1, type: socks5, server: 127.0.0.1, port: 1080}\n";string fixedNode=Mode2.SelectNode(raw,"Generic-1");Assert(!fixedNode.Contains("Generic-2"),"Fixed name depended on order");bool missing=false;try{Mode2.SelectNode(raw,"missing");}catch(InvalidOperationException){missing=true;}Assert(missing,"Missing fixed node accepted");Assert(Mode2.SelectNode(raw,"")==raw,"Unrequested city filter");
   var config=Core.Parse(Core.Build(fixedNode,new[]{edge},2));Assert(Core.Get(config,"sniffer")!=null,"Sniffer missing");Assert(Core.Str(Core.Get(config,"mode"))=="rule","Global mode enabled");
  }
  ...
  public static void MockTest(string report){
   string fixture=Path.Combine(Path.GetTempPath(),"ClashMode2Test-"+Guid.NewGuid().ToString("N"));string app=Path.Combine(fixture,"ByteDance","douyin","douyin-test.exe");Directory.CreateDirectory(Path.GetDirectoryName(app));File.WriteAllText(app,"Non-executable fixture used only for process rule validation");
   try{var log=new List<string>();foreach(int mode in new[]{0,2})foreach(bool socks in new[]{false,true})using(var proxy=new MockProxy(socks)){var prepared=Core.Prepare(new Input{Kind=socks?"SOCKS5":"HTTP",Address="127.0.0.1:"+proxy.Port,Username="test",Password="test",RuleMode=mode,Paths=new[]{mode==2?app:Core.Exe}},s=>{});Assert(prepared.TestIP=="203.0.113.77","Mock proxy response did not travel through Mihomo");log.Add("mode="+mode+" "+(socks?"SOCKS5":"HTTP")+": native Mihomo rule/sniffer validation, authenticated proxy handshake, HTTP body through proxy, exit parsing and cleanup PASS");File.WriteAllLines(report+".details",log);}File.WriteAllLines(report,log);}finally{Directory.Delete(fixture,true);}
  }
